...
const ruFttIcon = require('!!raw-loader!../../images/blips/frame6.inline.svg').default;
const notRuNoFttIcon = require('!!raw-loader!../../images/blips/frame5.inline.svg').default;
const ruNoFttIcon = require('!!raw-loader!../../images/blips/frame7.inline.svg').default;
...
function selectRadarQuadrant(order, startAngle, name) {
  ...
  const { x, y } = quadrantGroupTranslate[order]
  const xUnit = x ? 'px' : ''
  const yUnit = y ? 'px' : ''
  ...
  d3.select('.quadrant-group-' + order)
    ...
    .style('left', 'unset')
    .style('right', 0)
    .style('transform', `translate(${x}${xUnit}, ${y}${yUnit})`)
    // .attr('transform', `translate(${x}${xUnit}, ${y}${yUnit})`)
...
function wrapQuadrantNameInMultiLine(elem, isTopQuadrants, quadrantNameGroup, tip) {
  ...
  const testElWidth = testElem.getBoundingClientRect().width;

  if (testElWidth > maxWidth) {
...
function renderRadarQuadrants(size, svg, quadrant, rings, ringCalculator, tip) {
  ...
  rings.forEach(function (ring, i) {
    ...
    quadrantGroup
      .append('path')
      .attr('d', arc)
      .attr('class', `ring-arc-${ring.order()} ${highlightClass}`)
      .attr(
        'transform',
...
function renderRadarLegends(radarElement) {
  ...
  const ru = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(ruIcon);

  ru.select('svg')
    .attr('width', '32')
    .attr('height', '32')
    .append("svg:title").text("Решение из реестров отечественных решений")

  const notRu = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(notRuIcon);
  ...
  notRu.select('svg')
    .attr('width', '32')
    .attr('height', '32')
    .append("svg:title").text("Имеет санкционные риски")

  const notRuFtt = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(notRuFttIcon);
  ...
  notRuFtt.select('svg')
    .attr('width', '32')
    .attr('height', '32')
    .append("svg:title").text("Прошло апробацию, рекомендовано к использованию")

  const ruFtt = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(ruFttIcon);
  ...
  const notRuNoFtt = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(notRuNoFttIcon);

  notRuNoFtt.select('svg')
    .attr('width', '32')
    .attr('height', '32')
    .append("svg:title").text("Прошло апробацию. Не рекомендовано к использованию")

  const ruNoFtt = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(ruNoFttIcon);
  ...
  const defaultSvg = legendsContainer
    .append('div')
    .classed('flex content-center fill-blue', true)
    .html(defaultIcon);

  defaultSvg.select('svg')
    .attr('width', '32')
    .attr('height', '32')
    .append("svg:title").text("Иное")
}
