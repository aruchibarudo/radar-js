...
const { API_FIELDS, getAttribute } = require('../../util/apiUtils')
...
function renderBlipDescription(blip, ring, quadrant, tip, groupBlipTooltipText) {
  ...
  if (!groupBlipTooltipText) {
    ...
    const blipItemDivDescription = blipItemDiv.append('div')
      .classed('blip-list__item-container__description', true)
      .attr('id', `blip-description-${blip.id()}`)

    blipItemDivDescription
      // .html(`${blip.description()} ${blip.ru()}`)
      .append('p').text(blip.description())

    const blipItemDivTable = blipItemDivDescription.append('table')
    const rowHead = blipItemDivTable.append('tr');
    rowHead.append('th').text('Атрибут');
    rowHead.append('th').text('Описание');

    API_FIELDS.forEach(field => {
      const attr = getAttribute({ blip, field })

      /*blipItemDivTable
        .append('p').text(`${attr.name}: ${attr.description}`)*/

      const row = blipItemDivTable.append('tr');
