...
const notRuHasFttIcon = require('!!raw-loader!../images/blips/frame2.inline.svg').default;
...
function drawBlipInCoordinates(blip, coordinates, order, quadrantGroup) {
  ...
  group
    .append('text')
    .attr('x', blip.isGroup() ? (blip.isNew() ? 45 : 64) : xText)
    .attr('y', blip.isGroup() ? 17 : 25)
...
function createGroupBlip(blipsInRing, blipType, ring, quadrantOrder) {
  const blipText = `${blipsInRing.length} элементов`
  const blipId = `${quadrantOrder}-${replaceSpaceWithHyphens(ring.name())}-group-${replaceSpaceWithHyphens(
    blipType,
  )}-blips`

  const groupBlip = new Blip({ name: blipText, ring, isNew: blipsInRing[0].isNew() })
...
function plotGroupBlips({ ringIndex, ringBlips, ring, quadrantOrder, parentElement, quadrantWrapper, tooltip }) {
  let newBlipsInRing = [],
    existingBlipsInRing = []
...
const plotRadarBlips = function (parentElement, rings, quadrantWrapper, tooltip) {
  ...
  rings.forEach(function (ring, i) {
    ...
    if (ringBlips.length > graphConfig.maxBlipsInRings[i]) {
      plotGroupBlips({ ringIndex: i, ringBlips, ring, quadrantOrder, parentElement, quadrantWrapper, tooltip })
      return
    }
