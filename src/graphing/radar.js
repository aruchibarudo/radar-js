...
const { renderRadarsList } = require('./components/radarsList')
...
const Radar = function (size, radar) {
  ...
  function plotRingNames(quadrantGroup, rings, quadrant) {
    const getMaxLength = (i) => ringCalculator.getRingRadius(i) > 300 ? 10 : 15

    rings.forEach(function (ring, i) {
      const ringNameWithEllipsis = ring.name().length > getMaxLength(i) ? ring.name().slice(0, getMaxLength(i)) + '...' : ring.name()
  ...
  function renderFullRadar() {
    ...
    d3.select('.quadrant-subnav__dropdown-selector').text('Все квадранты')
  ...
  self.plot = async function () {
    ...
    _.each(quadrants, function (quadrant) {
      let quadrantGroup
      if (featureToggles.UIRefresh2022) {
        quadrantGroup = renderRadarQuadrants({ svg, quadrant, rings, ringCalculator, tip })
